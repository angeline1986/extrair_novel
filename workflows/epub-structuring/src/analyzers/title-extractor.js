...
function collectCandidates(doc, tocEntry, item) {
  const base = [doc?.heading, doc?.firstBold, tocEntry?.label, doc?.firstParagraph, doc?.title, item.href];
...
function scoreTitle(value) {
  ...
  if (/^bsj$/i.test(title) || /^\*+$/.test(title)) return 0;
  if (/converted ebook/i.test(title)) return 0;
  if (looksLikeChapterTitle(title)) return 100;
  if (isLikelyBodyText(title)) return 0;
  if (title.length <= 60 && title.split(' ').length <= 6) return 20;
  return 0;
}
