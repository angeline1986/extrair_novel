...
export function analyzeTitleQuality(title, doc, score = 0) {
  ...
  if (/^bsj$/i.test(normalized) || /^\*+$/.test(normalized)) return result('invalid', 'MARKER_NOT_TITLE');
  if (/converted ebook/i.test(normalized)) return result('invalid', 'GENERIC_TITLE');
  ...
  if (score >= 30 && (doc?.wordCount || 0) > 100) return result('suspicious', 'UNNUMBERED_POSSIBLE_TITLE');
