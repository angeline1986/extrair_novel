export function validateEpub3(structureReport, chapterReport, tocReport, languageReport) {
  ...
  if (chapterReport.sequence.missingChapters.length) {
  ...
  if (broken.length) issues.push(error('BROKEN_CHAPTERS', `${broken.length} documentos parecem capítulos sem título válido.`));

  const invalid = chapterReport.documents.filter((doc) => doc.titleQuality?.quality === 'invalid');
  if (invalid.length) issues.push(warn('INVALID_TITLES', `${invalid.length} documentos com título inválido.`));

  const suspicious = chapterReport.documents.filter((doc) => doc.titleQuality?.quality === 'suspicious');
